...
class Root extends React.Component<AppRouterPropsI, IAppRouterState> {
    ...
    getRender ( Component: React.ComponentClass, props: { history } | { location: string, context: Object} ): JSX.Element {
        return (
    ...
    render() {
        if ( this.isBrowser ) {
            return (
                <MuiThemeProvider {...{ theme }}>
                    { this.getRender( Router, { history }) }
                </MuiThemeProvider>
            );
        } else {
            /** If ssr then the MuiThemeProvider is rendered on server side (in template) */
            return (
                <React.Fragment>
                    { this.getRender( StaticRouter, { location: '/', context: {} }) }
                </React.Fragment>
            );
        }
    }
